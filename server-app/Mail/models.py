...
class Mail(object):
    ...
    def regis_verify(mail, code, fail_silently=True):
        html_message = Mail.get_header()\
                       ...
                       + Mail.get_footer()
        send_mail("Food For All - Your verification code", "", EMAIL_HOST_USER, [mail], fail_silently=fail_silently, html_message=html_message)
    ...
    def reset_password_verify(mail, code, fail_silently=True):
        html_message = Mail.get_header()\
                       ...
                       + Mail.get_footer()
        send_mail("Food for All - Your verification code for password reset", "", EMAIL_HOST_USER, [mail], fail_silently=fail_silently, html_message=html_message)
    ...
    def reset_password_success(mail, fail_silently=True):
        html_message = Mail.get_header()\
                       ...
                       + Mail.get_footer()
        send_mail("Food For All - You have reset your password", "", EMAIL_HOST_USER, [mail], fail_silently=fail_silently, html_message=html_message)
    ...
    def share(share_info, fail_silently=True):
        ...
        html_message = Mail.get_header()\
                       ...
                       + Mail.get_footer()
        send_mail("Food For All - Invitation to make the world a little better", "", EMAIL_HOST_USER, share_info["mail"], fail_silently=fail_silently, html_message=html_message)
